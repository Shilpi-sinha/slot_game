...
function RTPcalc(nrSpins) {
    ...
    for (var i = 1; i <= nrSpins; i++) {
        if (i % (nrSpins / 100) == 0) {
            console.clear();
            console.log("".concat((i / nrSpins * 100).toFixed(2), "%"));
        }
        var temp = new Spin(bet, false);
        balance = balance - bet;
        if (temp.totalWin > hw) {
            highestWin = "Highest win was ".concat(temp.totalWin / bet, "x at spin number ").concat(i, ".");
            hw = temp.totalWin;
        }
        if (temp.totalWin > 0) {
    ...
    console.log("Done ".concat(spins, " spins with bet ").concat(bet, ". \n").concat(highestWin, " \nBalance at end is ").concat(balance, ".\nThat means RTP is ").concat(balance / 10000, "%"));
}
        ...
        for (var ro = 0; ro < slot.rows; ro++) {
            ...
            for (var re = 0; re < slot.reels; re++) {
                ...
                if (res != undefined) {
                    ...
                    throw new Error("Error drawing value res (".concat(res, ") is undefined"));
                }
            }
        }
        return result;
